const favoriteOrdersURL = "http://localhost:3000/favorite_orders/";
...
function identifyFavorite(favoriteOrderID) {
    ...
    let favoriteOrderPromise = $.get("http://localhost:3000/favorite_orders/" + favoriteOrderID);
    ...
    if (newFavoriteName && isValidOrder(favoriteOrderIngredients)) {
        ...
        $.ajax({
            ...
            success: function (updated_favorite_order) {
                getFavorites();
                showAlert(updated_favorite_order.favorite_name)
            }
        });
